...
function guardaCosto() {
    ...
    get('/ws/CTransporte.aspx/' + url, JSON.stringify({ info: JSON.stringify(costo) }))
        .then(function (res) {
            if (res.Respuesta === 1) {
...
function editaCosto(id) {
    get('/ws/CTransporte.aspx/Consultar', JSON.stringify({ id: id }))
        .then(function (res) {
            if (res.Respuesta === 1) {
...
function cargaCtransporte(fil = "") {
    ...
    var data = {
            ...
            },
            {
                propiedad: '', class: 'text-center tdp', formato(tr, obj) {
                    ...
                    $(elimina).addClass("fa fa-trash text-danger").prop("title", "Elimina").on("click", function () {
                        eliminaCosto(obj.ID_TRANSPORTE);
                    });
        ...
        url: '/ws/CTransporte.aspx/Listar',
    ...
    $("#ctransporte").MALCO(data);
}
function eliminaCosto(id) {
    get('/ws/CTransporte.aspx/Eliminar', JSON.stringify({ id: id }))
        .then(function (res) {
            if (res.Respuesta === 1) {
                cargaCtransporte();
