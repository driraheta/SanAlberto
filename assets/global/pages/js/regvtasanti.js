                ...
                if (res.Respuesta === 1) {
                    if (res.Info !== null) {
                        $.each(res.Info, function () {
                            ...
                            $("#fpe").append('<option value="' + this.id + '">' + this.con + '</option>');
                        });
                    }
                }
                else {
        ...
        get('/ws/Clientes.aspx/ConsultarC', JSON.stringify({ id: id }))
            .then(function (res) {
                if (res.Respuesta === 1) {
                    if (res.Info !== null) {
                        $.each(res.Info, function () {
                            $("#clie").append('<option value="' + this.id + '">' + this.nom + '</option>');
                            $("#clie").val(idd);
    ...
    function cargaDireccionLlegadae(id,idd) {
        get('/ws/Clientes.aspx/ConsultarDir', JSON.stringify({ id: id }))
            .then(function (res) {
                if (res.Respuesta === 1) {
                    if (res.Info !== null) {
                        $(res.Info).each(function () {
                            $("#dllege").append('<option value="' + this.id + '">' + this.desc + '</option>');
                            $("#dllege").val(idd);
    ...
    function cargaOrdenes(fil = "") {
        ...
        var data = {
            ...
            columnas: [
                ...
                { leyenda: 'Stock Vta. Ant.', class: 'text-center thp', ordenable: false, columna: 'STOCKVTASANTI', filtro: true }
            ],
            modelo: [
                { propiedad: 'NUMERO', class: 'text-center tdp' },
                ...
                { propiedad: 'STOCKVTASANTI', class: 'text-center tdp' }
            ],
    ...
    function cargaVentas(id,fecha) {
        $("#infoven tbody").empty();
        get('/ws/RegVtas.aspx/Listar', JSON.stringify({ id: id,tipo:"PE" }))
            .then(function (res) {
                if (res.Respuesta === 1) {
                    $(res.Info).each(function () {
                        let fila = "";
    ...
    function cargaVentAnti(fil = "") {
        ...
        var data = {
            ...
            columnas: [
                ...
                { leyenda: 'CLIENTE', class: 'text-center thp', ordenable: false, columna: 'DESCRIPCION', filtro: true },
                { leyenda: 'FECHA EMISIÓN', class: 'text-center thp', ordenable: false, columna: 'CANTIDAD', filtro: true },
                { leyenda: 'IMPORTE', class: 'text-center thp', ordenable: false, columna: 'STOCK', filtro: true },
                { leyenda: 'VER DETALLE', class: 'text-center thp', ordenable: false, columna: '', filtro: true }
            ],
            modelo: [
                {
            ...
            .then(function (res) {
                ...
                if (res.Respuesta === 1) {
                    if (res.Info !== null) {
                        ...
                        cargaDireccionLlegadae(res.Info.vent.cli, res.Info.vent.dirlleg);
    ...
    function EditarRegistro() {
        ...
        $.each($("#productose tbody tr"), function () {
            ...
            det.idalm = $(this.cells[11]).attr("almc");
    ...
    $(document).ready(function () {
        ...
        $("#nue").on("click", function () {
            ...
            $("#fp option").each(function () {
                if ($(this).text().toUpperCase() === "CONTADO") {
        ...
        $("#tpbody").on('input', '.cantt', function () {
            var pre = $(this).parent().next().next().children().val();
            var ivg = $(this).parent().next().next().next().next().children().val().replace("%", "");
            $trSub = $(this).parent().next().next().next().children();
            $trTot = $(this).parent().next().next().next().next().next().children();
            console.log(Number($(this).val()));
            console.log(ivg);
            console.log($(this).parent().next().next().next().next().next().children().val());
