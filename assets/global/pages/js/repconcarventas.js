(function ($) {
    ...
    $("#fechaFin").val(hoy());
    //Creamos el Tbody
    function TbodyVentaPorCliente(fechaIni = hoy(), fechaFin = hoy(), presionado = false) {
        console.log("FECHA INI: " + fechaIni);
        console.log("FECHA FIN: " + fechaFin);
       
        get('/ws/Ventas.aspx/GenerarReporteConcarVentas', JSON.stringify({ fechaIni: fechaIni, fechaFin: fechaFin }))
            .then(function (res) {
                ...
                if (r.startsWith('[{"ERROR":', 0)) {
                    ...
                    $(res).each(function () {
                        ...
                        let ID_CONCARVENTA = (this.ID_CONCARVENTA != null) ? this.ID_CONCARVENTA : 0;
                        let ID_VENTAS = (this.ID_VENTAS != null) ? this.ID_VENTAS : 0;
                        ...
                        //INICIO FILA
                        tableHTML += "<tr  idVenta='" + ID_VENTAS + "' idConcarVenta='" + ID_CONCARVENTA + "' idUsuario='" + ID_USUARIO + "' class='text-center justify-content-center'>";
                        //INICIO COLUMNAS
                        tableHTML += "<td>" +
                    ...
                    $('#tblRepConcarVentas table tbody').html(tableHTML);
                    $('#btnFiltro').show();
                }
    ...
    function ReporteConcarVentasExportarExcel(tipo) {
        var jsondet = [];
        var tpon = "";

        jsondet = obtenerFilaTabla("tblRepConcarVentas", tipo);
        ...
        if (jsondet.length > 0) {
            ...
            get('/ws/ventas.aspx/ReporteConcarVentasExportarExcel', JSON.stringify(json))
                .then(function (res) {
                    var r = JSON.stringify(res);
                    r = r.replace("\"", "");
                    r = r.replace("\"", "");
    ...
    function limpiarExcel() {
        ...
        get('/ws/ventas.aspx/LimpiarReporteExcel')
            .then(function (res) {
                var r = JSON.stringify(res);
    ...
    $('#btnFiltro').on('click', function myfunction() {
        ...
        if (validarIntervaloFechas()) {
            ...
            TbodyVentaPorCliente(fechaIni, fechaFin, true);
        } else {
            $('#btnFiltro').show();
        }
    });
    ...
    $('#btnExportarExcel').on('click', function myfunction() {
        ...
        if (validarRegistrosTabla()) {
            ReporteConcarVentasExportarExcel("All");
    ...
    function obtenerFilaTabla(idDivTabla, tipo) {
        ...
        $("#" + idDivTabla + " table tbody").children("tr").each(function (index) {
            ...
            var idConcarVenta = $(this).attr("idConcarVenta");
            var idUsuario = ($(this).attr("idUsuario") == 0) ? usuario : $(this).attr("idUsuario");

            console.log(idVenta);
            console.log(idConcarVenta);
    ...
    $("#fechaIni").val(hoy());
    $("#fechaFin").val(hoy());
    $('#tblRepConcarVentas table tbody').html("");
