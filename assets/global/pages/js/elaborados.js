//Variables globales para almacenamiento temporal de datos
...
$(document).ready(function () {
    ...
    $('#fregistrosal').datepicker({ calendarWeeks: false, uiLibrary: 'bootstrap4', format: "dd/mm/yyyy", defaultDate: new Date(), value: formatoFecha(new Date(), 1) });
...
function salida() {
    detProdArray.length = 0;
    limpiaControles("salida");
    $("#lista").hide();
    $("#salida").show();
    $("#almeente").prop("selectedIndex", 0);
            ...
            if (res.Respuesta === 1) {
                if (res.Info !== null) {
                    $(res.Info).each(function () {
                        ...
                        $("#selprode").append('<option value="' + this.ID_PRODUCTO + '">' + this.PRODUCTO + '</option>');
...
function cargaInvElaborados() {
    ...
    get('/ws/elaborados.aspx/Listar', JSON.stringify({ desde: null, hasta: null, idalm: alm }))
        .then(function (res) {
            var r = JSON.stringify(res);
            $("#info tbody").html("");
            if (r.startsWith('[{"Error":', 0)) {
                ...
                $(res.Info).each(function () {
                    ...
                    html += "<td>" + formatoMoneda(this.COSTO_UNITARIO, 2, true) + "</td>";
                    //html += "<td>" + formatoMoneda(this.costo, 2, true) + "</td>";
                    html += "<td>" + this.CANTIDAD + "</td>";
                    html += "</tr>";
                    i++;
                    //}
                });
...
function exportar(tipo) {
    ...
    if (jsondet.length > 0) {
        var json = {
            ...
            vista: tpon,
            formato: 'FormatoInvElaborado',
            vercos: verco
        };
        get('/ws/inventarios.aspx/ExportaExcel', JSON.stringify(json))
            .then(function (res) {
                //var r = JSON.stringify(res);
...
function genpdf() {
    ...
    if (jsondet.length > 0) {
        ...
        get('/ws/inventarios.aspx/GeneraPdfInvElab', JSON.stringify(json))
            .then(function (res) {
                if (res.Respuesta === 1) {
...
function calculaStockOC(posicion) {
    ...
    let cantActualOC = bodyOC.children()[posicion - 1].children[6].innerHTML;
    let SaldoOC = parseFloat(cantActualOC) - parseFloat(cantidadOC);
    bodyOC.children()[posicion - 1].children[8].innerHTML = SaldoOC;
    let sumCantOc = 0;
...
function cargaArrayProdsInsumo() {
    ...
    while (++i <= cantRows) {
        ...
        if ($.trim(localVal) != '') {
            ...
            registro.tipoOrden = bodyOC.children()[i - 1].children[2].innerHTML;
            registro.cantidad = localVal;
            registro.saldoAnt = bodyOC.children()[i - 1].children[6].innerHTML;
            registro.saldoNuevo = bodyOC.children()[i - 1].children[8].innerHTML;
            registro.Precio = bodyOC.children()[i - 1].children[9].innerHTML;
...
function guardaRegistroSalida() {
    //Variables
    let reg = new Object();
    let idalm = $("#selalme").val();
    let detalle = new Array();
    let detalleoc = new Array();
    let registro = new Object();
    let observacion = $("#txtobse").val();

    //Datos del producto elaborado	 
    let idElaborado = $("#selprode").val();
    let cantElaborado = $("#cantsal").val();
    let cunitElaborado = $("#cunitsal").val();

    var from = $("#fregistrosal").val().split("/");
    let fecElaborado = new Date(from[2], from[1] - 1, from[0]);
    let umElaborado = 'UNIDAD';

    //DATOS DE COMPOSICION (Pendiente)
    ...
    Swal.fire({
        ...
        showLoaderOnConfirm: true,
        preConfirm: () => {
            return fetch(`/ws/elaborados.aspx/InsertarEgreso`, {
                method: 'POST',
                body: JSON.stringify({ info: JSON.stringify(registro) }),
                headers: { 'Content-Type': 'application/json' }
            })
