...
function cargaCostos(fil="") {
    ...
    var data = {
        ...
        modelo: [
            {
                propiedad: 'ALMACENAJE', class: 'text-center px-2 tdp', style: "white-space:nowrap", formato: function (tr, obj, valor) {
                    if (valor === 1)
                        return "Costo Almacenaje";
                    if (valor === 2)
                        return "Desestiba y Estiba";
                    if (valor === 3)
            ...
            },
            {
                propiedad: '', class: 'text-center tdp', formato(tr, obj) {
                    ...
                    $(elimina).addClass("fa fa-trash text-danger").prop("title", "Elimina").on("click", function () {
                        eliminaCosto(obj.ID_ALMACENAJE);
                    });
        ...
        url: '/ws/CAlmacenaje.aspx/Listar',
...
function guardaCosto() {
    ...
    get('/ws/CAlmacenaje.aspx/' + url, JSON.stringify({ info: JSON.stringify(costo) }))
        .then(function (res) {
            if (res.Respuesta === 1) {
        ...
        .catch(function (error) {
            Alerta("no fue posible insertar la información del costo de almacenaje<br />" + error, "ERROR!");
        });
}
function editaCosto(id) {
    get('/ws/CAlmacenaje.aspx/Consultar', JSON.stringify({ id: id }))
        .then(function (res) {
            if (res.Respuesta === 1) {
                if (res.Info.length > 0) {
                    $("#lista").hide();
                    $("#datos").show();
                    $("#tipa").val(res.Info[0].tipa);
...
function eliminaCosto(id) {
    get('/ws/CAlmacenaje.aspx/Eliminar', JSON.stringify({ id: id }))
        .then(function (res) {
            if (res.Respuesta === 1) {
                cargaCostos();
